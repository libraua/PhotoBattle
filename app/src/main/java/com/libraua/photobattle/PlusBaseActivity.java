...
public abstract class PlusBaseActivity extends Activity
    ...
    private static final int ACCOUNT_REQUEST_CODE = 49404;
    ...
    public void signIn() {
        if (!mGoogleApiClient.isConnected()) {
            ...
            if (mConnectionResult != null) {
                Log.d("PhotoBattle", "It's not a first run");
                startResolution();
            } else {
    ...
    private void startResolution() {
        try {
            ...
            // being started here.
            log("startResolution");
            mConnectionResult.startResolutionForResult(this, ACCOUNT_REQUEST_CODE);
        } catch (IntentSender.SendIntentException e) {
            log("startResolution:" + e.toString());
    ...
    protected void onActivityResult(int requestCode, int responseCode, Intent intent) {
        updateConnectButtonState();
        if (requestCode == ACCOUNT_REQUEST_CODE && responseCode == RESULT_OK) {
            // If we have a successful result, we will want to be able to resolve any further
            // errors, so turn on resolution with our flag.
            mAutoResolveOnFail = true;
            // If we have a successful result, let's call connect() again. If there are any more
            // errors to resole we'll get our onConnectionFailed, but if not,
            // we'll get onConnected.
            Log.d(TAG, "Result ok");
            initiatePlusClientConnect();
        } else if (requestCode == ACCOUNT_REQUEST_CODE && responseCode != RESULT_OK) {
